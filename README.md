...
3. **Фильтрация (Junior-фильтр):** кандидаты уровня **Junior** автоматически отсеиваются на отдельный лист «Junior» для экономии токенов. Кандидаты **Middle/Senior** проходят дальше.
...
![Сценарий 1](workflow-1.png)

### Сценарий №2: Автоматическая обратная связь
![Сценарий 2](workflow-2.png)

---

## 🚀 Инструкция по развертыванию
Поскольку Albato не поддерживает прямой импорт файлов сценариев, воспользуйтесь готовыми конфигурациями из папки `prompts/` для воссоздания MVP:

1. **Создание сценариев:** В аккаунте Albato создайте два сценария согласно схемам выше.
2. **Настройка GigaChat:**
   * Для первого сценария (Экстракция) используйте промпт из файла `prompts/prompt-extractor.txt`.
   * Для второго сценария (Метчинг) используйте промпт из файла `prompts/prompt-matching.txt`.
3. **Декодирование данных:** Для настройки инструмента «Декодировать JSON» используйте структуру полей из файлов:
   * `prompts/albato-settings-step1.txt` (для первого этапа).
   * `prompts/albato-settings-step2.txt` (для этапа со скорингом).
4. **Подготовка таблиц:** В Google Sheets создайте листы `Вакансии`, `Кандидаты` и `Junior`.
